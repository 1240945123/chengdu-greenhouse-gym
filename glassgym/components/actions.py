...
class NamedControlActionScheme:
    ...
    def commit_full_control_input(self, executed: np.ndarray) -> np.ndarray:
        executed_array = np.asarray(executed, dtype=np.float32)
        if executed_array.shape != (self.nu,) or not np.all(np.isfinite(executed_array)):
            raise ValueError("Executed control input must be a finite full control vector")
        self._prev_u_full = executed_array.copy()
        return self._prev_u_full.copy()
