...
OPTIMIZER = {"ADAM": Adam}
ENVS = {
    "GreenLightEnv": GreenLightEnv,
    "ChengduSingleGreenhouseEnv": GreenLightEnv,
}
...
def load_model_hyperparams(algorithm: str, env_id: str) -> Dict[str, Any]:
    with open(join("configs/agents/", algorithm + ".yml"), "r", encoding="utf-8") as f:
        params = yaml.load(f, Loader=yaml.FullLoader)
    model_hyperparams = params[env_id] if env_id in params else params["GreenLightEnv"]
...
def load_env_params(env_id: str, path: str) -> Dict[str, Any]:
    ...
    with open(join(path, env_id + ".yml"), "r", encoding="utf-8") as f:
        params = yaml.load(f, Loader=yaml.FullLoader)
    env_kwargs = params[env_id] if env_id in params else params["GreenLightEnv"]
    return env_kwargs
...
def build_env_kwargs(env_kwargs):
    ...
    env_kwargs["weather_repository"] = WeatherRepository.from_config(
