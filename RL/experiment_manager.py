...
class ExperimentManager:
    ...
    def __init__(
        ...
        hp_tuning=False,
        device="cpu",
        dataset_id="amsterdam_reference",
    ):
        ...
        self.train_data_root = os.path.join("train_data", self.dataset_id, self.project)
    ...
    def initialise_model(self):
        ...
        if self.stochastic:
            tensorboard_log = os.path.join(self.train_data_root, self.algorithm, "stochastic", "logs", self.run.name)
        else:
            tensorboard_log = os.path.join(self.train_data_root, self.algorithm, "deterministic", "logs", self.run.name)
    ...
    def run_experiment(self):
        ...
        """
        if self.stochastic:
            model_log_dir = os.path.join(self.train_data_root, self.algorithm, "stochastic", "models", self.run.name) if self.save_model else None
            env_log_dir = os.path.join(self.train_data_root, self.algorithm, "stochastic", "envs", self.run.name) if self.save_env else None
        else:
            model_log_dir = os.path.join(self.train_data_root, self.algorithm, "deterministic", "models", self.run.name) if self.save_model else None
            env_log_dir = os.path.join(self.train_data_root, self.algorithm, "deterministic", "envs", self.run.name) if self.save_env else None
...
def main():
    ...
    parser.add_argument("--dataset_id", type=str, default="amsterdam_reference", help="Dataset namespace for local artifacts")
